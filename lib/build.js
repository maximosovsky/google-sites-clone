...
<body>${content}</body></html>`;
  ...
  // === Extract favicon from source site ===
  let faviconHtml = '';
  try {
    // Find favicon URL from first SingleFile page
    for (const p of pageMap.pages) {
      const sfPath = path.join(pagesDir, p.file);
      ...
      const iconMatch = sfHtml.match(/<link[^>]+rel="(?:shortcut )?icon"[^>]+href="([^"]+)"/i)
        || sfHtml.match(/<link[^>]+href="([^"]+)"[^>]+rel="(?:shortcut )?icon"/i);
      if (iconMatch) {
        const iconUrl = iconMatch[1];
        if (iconUrl.startsWith('data:')) {
          // Base64 favicon — decode and save
          const b64Match = iconUrl.match(/^data:[^;]+;base64,(.+)/);
          if (b64Match) {
            fs.writeFileSync(path.join(siteDir, 'favicon.ico'), Buffer.from(b64Match[1], 'base64'));
            faviconHtml = '<link rel="icon" href="favicon.ico">';
            console.log('  favicon.ico: extracted from base64 ✅');
          }
        } else if (iconUrl.startsWith('http')) {
          // Download favicon
          const res = await fetch(iconUrl, { redirect: 'follow' });
          if (res.ok) {
            const ext = iconUrl.includes('.png') ? 'png' : 'ico';
            const fname = `favicon.${ext}`;
            const buf = Buffer.from(await res.arrayBuffer());
            fs.writeFileSync(path.join(siteDir, fname), buf);
            faviconHtml = `<link rel="icon" href="${fname}">`;
    ...
    console.log(`  favicon: ⚠️ ${e.message}`);
  }
...
<aside class="sidebar">
  <div class="sidebar-header">${pageMap.pages[0]?.title || 'Site'}</div>
...
document.querySelector('.nav-item')?.classList.add('active');
  ...
  // Generate sitemap.xml (relative URLs — user must update with their domain)
  const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<!-- Update <loc> URLs with your domain before submitting to Search Console -->
