...
const { processVideos } = require('./video');
...
async function clone(url, options = {}) {
    try {
        ...
        if (options.maxPages && pageMap.pages.length > options.maxPages) {
            console.log(`⚠️  Limiting to ${options.maxPages} pages (found ${pageMap.pages.length})`);
            pageMap.pages = pageMap.pages.slice(0, options.maxPages);
        }
        ...
        await extractWithPuppeteer(pageMap, contentDir, { cooldown: options.cooldown });
        ...
        try { await reportError(err, { url, step: 'pipeline', page: null }); } catch (_) { }
        throw err;
    }
}
