...
function generateReport(dir) {
  ...
  const rows = pageMap.pages.map((p, i) => {
    ...
    // Video details for expandable row
    let videoDetail = '';
    if (ytIDs.length) videoDetail += ytIDs.map(id => `<span class="chip yt">▶ YT: ${id}</span>`).join(' ');
    if (vimeoIDs.length) videoDetail += vimeoIDs.map(id => `<span class="chip vm">▶ Vimeo: ${id}</span>`).join(' ');
    if (gdriveIDs.length) videoDetail += gdriveIDs.map(id => `<span class="chip gd">📁 GDrive: ${id.substring(0, 12)}…</span>`).join(' ');
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .chip { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 11px; margin: 2px; }
  .chip.yt { background: #fce8e6; color: #c5221f; }
  .chip.vm { background: #e8f0fe; color: #1a73e8; }
